...
function Home() {
  ...
  const startRecognition = () => {
    if (!isSpeakingRef.current && !isRecognizingRef.current) {
      try {
        ...
        isRecognizingRef.current = true; // ✅ Added
  ...
  const speak = (text) => {
    ...
    utterence.voice = voices.find(v => v.lang === 'hi-IN') || voices[0]; // ✅ fallback
    ...
    utterence.onend = () => {
      ...
      setTimeout(() => {
        startRecognition(); // ✅ restart safely after speaking
      }, 800);
    };
  ...
  useEffect(() => {
    ...
    const startTimeout = setTimeout(() => {
      if (isMounted && !isSpeakingRef.current && !isRecognizingRef.current) {
        try {
          recognition.start();
          isRecognizingRef.current = true; // ✅ mark active
    ...
    recognition.onresult = async (e) => {
      ...
      // ✅ Removed strict assistant name check for better usability
      setAiText("");
